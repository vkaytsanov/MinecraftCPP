...
void WorldGeneration::generateChunkContents(Chunk* chunk, int16_t i, int16_t j) {
	const int CHUNK_WORLD_POSITION_X = i * CHUNK_SIZE_X;
	const int CHUNK_WORLD_POSITION_Z = j * CHUNK_SIZE_Z;

	generateBiomeMap(chunk, CHUNK_WORLD_POSITION_X, CHUNK_WORLD_POSITION_Z);
...
void WorldGeneration::generatePrefabs(Chunk* chunk) {
	ChunkContentsPtr chunkData = chunk->getChunkContents();
	HeightMap* heightMap = &chunk->m_heightMap;
	BiomeMap* biomeMap = &chunk->m_biomeMap;
			...
			if (cube->m_type == Air) {
				if (chunkData->at(x).at(y - 1).at(z).isOpaque()) {
					...
					for (auto& prefab : m_biomes[biomeMap->at(x).at(z)].m_prefabs) {
						int freq = randomGenerator.getRandom(prefab.second);
						if (freq == 0) {
			...
			if (cube->m_type == Air) {
				if (chunkData->at(x).at(y - 1).at(z).isOpaque()) {
					for (auto& model : m_biomes[biomeMap->at(x).at(z)].m_models) {
						int freq = randomGenerator.getRandom(model.second);
						if (freq == 0) {
...
void WorldGeneration::generateBiomeMap(Chunk* chunk, int16_t wPosX, int16_t wPosZ) {
	...
	for (int x = 0; x < CHUNK_SIZE_X; x++) {
		for (int z = 0; z < CHUNK_SIZE_Z; z++) {
			...
			const float CHUNK_TEMPERATURE = m_temperatureGenerator.GetValue(globalPosX, globalPosY,0.8f);
			const float CHUNK_HUMIDITY = m_humidityGenerator.GetValue(globalPosX, globalPosY, 0.8f);

			biomeMap->at(x).at(z) = getBiomeType(CHUNK_TEMPERATURE, CHUNK_HUMIDITY);
